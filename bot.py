...
📸 Найти по QR:
- Гость показывает QR
- Отправляете фото QR в этот чат  
- Нажимаете кнопку '✔ Начислить'

📞 Найти по номеру:
- Гость говорит номер
- Отправляете номер в этот чат в формате: `9998887766 Имя` (без 8)
- Нажимаете кнопку '✔ Начислить'
    """
        ...
        if " " in text:
            try:
                ...
                if phone.isdigit() and len(phone) == 10:
                    ...
                    await update.message.reply_text(f"✅ Ваш профиль обновлен: {name} ({phone})")
                    set_user_state(context, 'client_mode')
                    await show_client_main(update, context)
                else:
                    await update.message.reply_text("❌ Номер должен быть 10 цифр")
